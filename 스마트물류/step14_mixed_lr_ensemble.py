...
# Step 14: Step12 피처셋 + 모델별 최적 학습률
# =============================================================================
# Step12 대비 변경점 (피처 동일, 학습률만 조정):
#
# Step13 분석 결과:
#   - LGB: lr 0.05→0.02 시 OOF 8.6289→8.6203 (-0.009 개선)
#   - XGB: lr 0.05→0.02 시 OOF 8.6283→8.6394 (+0.011 악화)
#   - CatBoost: lr 0.05→0.02 시 OOF 8.6351→8.6368 (+0.002 악화)
#
# → LGB만 lr=0.02 / MAX_ROUNDS=5000 / EARLY_STOP=200 으로 변경
# → XGB, CatBoost는 Step12 원본(lr=0.05)으로 유지
# → 피처는 Step12와 완전 동일 (300개)
#
# 실행:
#   caffeinate -i nohup python step14_mixed_lr_ensemble.py > step14_output.log 2>&1 &
#   tail -f step14_output.log
# =============================================================================
...
print(f"  Step14 OOF MAE: {best_mae:.4f}  ({len(feature_cols)} 피처, LGB lr=0.02 / XGB+Cat lr=0.05)")
print(f"  Step12→14 변화: {best_mae - 8.6050:+.4f}")
print(f"\n  Step12 Public: 10.0742  (현재 최고)")
print(f"  Step14 Public: 미제출 (예상 ~{best_mae + 1.469:.4f})")  # gap 1.469 (Step12 실측)
...
out = os.path.join(OUTPUT_DIR, 'submission_step14_mixed_lr_ensemble.csv')
submission.to_csv(out, index=False)
print(f"\n제출 파일: submission_step14_mixed_lr_ensemble.csv")
