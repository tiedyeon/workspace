# =============================================================================
# Step 49: Erlang-C 대기행렬 피처 + LGB 5seeds + XGB 3seeds 앙상블
# =============================================================================
# 핵심 아이디어:
...
#
# 베이스: step35 (OOF 8.5323, Public 10.0533, 570 피처)
# 개선 내용:
#   1) LEAD_COLS 11→18개 (near_collision, fault_count, robot_charging 등 추가)
#   2) lead NaN → 현재값 fill (마지막 슬롯 노이즈 제거)
#   3) 3개 핵심 컬럼 lead NaN 플래그 추가
#   4) lead4, lead5 (상위 5개 컬럼만, 75분 앞까지)
#   5) lead_max3, lead_mean3 cross 피처 (t+1~3 요약)
#   6) 복합 위험 cross 피처 3개 추가
#
# 실행:
#   caffeinate -i nohup python -u -B step49_ensemble.py > step49_output.log 2>&1 &
#   tail -f step36_output.log
# =============================================================================
...
print(f"Step49: Erlang-C 피처 + LGB 5seeds + XGB 3seeds + CAT 3seeds 앙상블")
print(f"Seeds: LGB {len(SEEDS)}개 + XGB 3개 + CAT 3개 × 5folds")
print(f"Seeds: {len(SEEDS)}개 × LGB only = {len(SEEDS)*N_SPLITS}모델")
...
AGG_COLS  = ['order_inflow_15m', 'low_battery_ratio', 'congestion_score',
             ...
             # step45 신규: 기존 LEAD_COLS에만 있던 피처들 → sc_* 집계 추가
             'fault_count_15m', 'near_collision_15m', 'battery_std',
             'avg_recovery_time', 'blocked_path_15m']
...
LEAD_COLS = [
    ...
    # step36 신규 (7개)
    'near_collision_15m', 'task_reassign_15m', 'fault_count_15m',
    'robot_charging', 'avg_charge_wait',
    'loading_dock_util', 'staging_area_util',
]

# lead4/lead5 적용할 상위 5개 컬럼 (타겟과 상관 높은 것)
LEAD_EXTENDED_COLS = [
    'congestion_score', 'low_battery_ratio', 'order_inflow_15m',
    'charge_queue_length', 'max_zone_density',
]
REMAIN_COLS = [
    ...
    'near_collision_15m', 'fault_count_15m',
]
...
def feature_engineering(df):
    ...
    # [Step48] layout_info 파생 피처 (test.csv 미사용, layout_info.csv raw 컬럼만 활용)
    if 'charger_count' in df.columns and 'robot_total' in df.columns:
        df['charger_per_robot']     = df['charger_count'] / (df['robot_total'] + 1)
        df['charger_density']       = df['charger_count'] / (df['floor_area_sqm'] + 1)
    if 'pack_station_count' in df.columns:
        df['pack_per_robot']        = df['pack_station_count'] / (df['robot_total'] + 1)
        df['pack_density']          = df['pack_station_count'] / (df['floor_area_sqm'] + 1)
    if 'num_aisles' in df.columns:
        df['area_per_aisle']        = df['floor_area_sqm'] / (df['num_aisles'] + 1)
        df['robot_per_aisle']       = df['robot_total'] / (df['num_aisles'] + 1)
        df['charger_per_aisle']     = df['charger_count'] / (df['num_aisles'] + 1)
    if 'num_zones' in df.columns:
        df['robot_per_zone']        = df['robot_total'] / (df['num_zones'] + 1)
        df['area_per_zone']         = df['floor_area_sqm'] / (df['num_zones'] + 1)
    if 'charger_count' in df.columns and 'pack_station_count' in df.columns:
        df['infra_ratio']           = df['charger_count'] / (df['pack_station_count'] + 1)
        df['total_infra']           = df['charger_count'] + df['pack_station_count']
        df['infra_per_robot']       = df['total_infra'] / (df['robot_total'] + 1)
    # layout_info × 운영 상태 cross 피처
    if 'charger_per_robot' in df.columns:
        df['charge_pressure_norm']  = df['charge_queue_length'] / (df['charger_per_robot'] + 1e-5)
    if 'pack_per_robot' in df.columns:
        df['pack_pressure_norm']    = df['pack_utilization'] / (df['pack_per_robot'] + 1e-5)
    if 'robot_per_aisle' in df.columns:
        df['congestion_norm']       = df['congestion_score'].fillna(0) / (df['robot_per_aisle'] + 1e-5)
    # [Step49] Erlang-C 대기행렬 이론 피처
    # M/M/c 모델: c개 서버(charger), 도착률 λ(charge_queue_length), 서비스율 μ(1/avg_charge_wait)
    if 'charger_count' in df.columns and 'charge_queue_length' in df.columns:
        c   = df['charger_count'].fillna(1).clip(lower=1)
        lam = df['charge_queue_length'].fillna(0)
        mu  = 1.0 / (df['avg_charge_wait'].fillna(1).clip(lower=0.1))
        rho = lam / (c * mu + 1e-9)                          # 서버 이용률
        df['erlang_rho']          = rho.clip(0, 10)          # 포화도 (1이상=위험)
        df['erlang_overload']     = (rho - 1).clip(0, None)  # 초과 부하량
        df['erlang_slack']        = (1 - rho).clip(0, None)  # 여유 용량
        # 이론적 대기시간 근사 (rho < 1 구간만 유효, rho >= 1은 발산)
        stable = rho < 0.99
        erlang_wait = np.where(stable, rho / (c * mu * (1 - rho) + 1e-9), 999.0)
        df['erlang_wait_theory']  = np.clip(erlang_wait, 0, 999)
        # 실제 대기 - 이론 대기 (모델이 이론에서 얼마나 벗어났는지)
        df['erlang_wait_gap']     = df['avg_charge_wait'].fillna(0) - df['erlang_wait_theory'].clip(0, 100)
    # 패킹 스테이션 대기행렬 피처
    if 'pack_station_count' in df.columns and 'pack_utilization' in df.columns:
        c_pack  = df['pack_station_count'].fillna(1).clip(lower=1)
        rho_pack = df['pack_utilization'].fillna(0) / 100.0
        df['pack_rho']        = rho_pack.clip(0, 10)
        df['pack_overload']   = (rho_pack - 0.9).clip(0, None)  # 90% 이상 위험
        df['pack_erlang_est'] = rho_pack / (c_pack * (1 - rho_pack.clip(0, 0.99)) + 1e-9)
    # [Step45 신규] capacity-normalized sc 피처
    # sc_order_inflow / pack_station_count: 창고 용량 대비 주문 부하 (상관 0.4389)
    # sc_*는 build_sc_features()에서 나중에 merge되므로 여기서는 임시 계산
    sc_oi_mean_tmp = df.groupby('scenario_id')['order_inflow_15m'].transform('mean')
    sc_oi_max_tmp  = df.groupby('scenario_id')['order_inflow_15m'].transform('max')
    df['sc_oi_mean_per_pack'] = sc_oi_mean_tmp / (df['pack_station_count'].fillna(1) + 1)
    df['sc_oi_max_per_pack']  = sc_oi_max_tmp  / (df['pack_station_count'].fillna(1) + 1)
    return df
...
def add_aggregation_features(train_df, test_df):
    """
    [Step48 규칙 준수]
    - sc_* 집계: train/test 각각 독립 계산 (test 데이터가 train 학습에 개입 없음)
    - layout_agg 완전 제거: train 시나리오 기반 layout 통계는 Adversarial AUC=1.0 원인이자
      간접적으로 test 분포를 학습에 끌어들이는 구조 → 규칙 위반 가능성 → 원천 제거
    - layout_info.csv raw 컬럼은 step1에서 이미 merge됨 → feature_engineering()에서 파생 피처 생성
    """
    # sc_* 집계 (각 시나리오 내 통계, train/test 완전 독립)
    ...
    print(f"  [Step48] layout_agg 제거 완료 (Adversarial 피처 원천 차단)")
    print(f"  sc_* 집계: train {len(train_sc)}개 시나리오, test {len(test_sc)}개 시나리오 (독립 계산)")
...
def add_lead_features(df):
    """
    [Step36] 미래 슬롯 직접값 및 변화 방향 피처 (확장 버전)
    개선:
      - NaN → 현재값 fill (마지막 슬롯 노이즈 제거)
      - 3개 핵심 컬럼 NaN 플래그 (모델이 "미래 없음" 학습)
      - LEAD_COLS 18개 (step35 11개 → 7개 추가)
      - lead4/lead5 (상위 5개 컬럼, 75분 앞까지)
      - lead_max3, lead_mean3 (t+1~3 요약 통계)
      - 복합 위험 cross 피처 추가
    """
    df = df.copy()
    grp = df.groupby('scenario_id', sort=False)

    # 마지막 슬롯 플래그 (모델에 컨텍스트 제공)
    sc_max_slot = grp['slot_idx'].transform('max')
    df['is_final_slot']       = (df['slot_idx'] == sc_max_slot).astype(np.int8)
    df['slots_to_scenario_end'] = (sc_max_slot - df['slot_idx']).astype(np.float32)

    for col in LEAD_COLS:
        if col not in df.columns:
            continue
        cur   = df[col]
        lead1_raw = grp[col].shift(-1)
        lead2_raw = grp[col].shift(-2)
        lead3_raw = grp[col].shift(-3)

        # [NEW] 핵심 3개 컬럼만 NaN 플래그 (feature 수 절약)
        if col in ['congestion_score', 'low_battery_ratio', 'order_inflow_15m']:
            df[f'lead1_nan_{col}'] = lead1_raw.isna().astype(np.int8)

        # [NEW] NaN → 현재값 fill (마지막 슬롯 "상태 유지" 가정)
        lead1 = lead1_raw.fillna(cur)
        lead2 = lead2_raw.fillna(lead1)
        lead3 = lead3_raw.fillna(lead2)
        ...
        # 변화 방향
        df[f'lead_diff1_{col}'] = lead1 - cur
        df[f'lead_diff2_{col}'] = lead2 - lead1
        df[f'lead_accel_{col}'] = (lead2 - lead1) - (lead1 - cur)

    # [NEW] lead4/lead5: 상위 5개 컬럼 (75분 앞까지)
    for col in LEAD_EXTENDED_COLS:
        if col not in df.columns:
            continue
        lead3 = df.get(f'lead3_{col}', grp[col].shift(-3).fillna(df[col]))
        lead4_raw = grp[col].shift(-4)
        lead5_raw = grp[col].shift(-5)
        lead4 = lead4_raw.fillna(lead3)
        lead5 = lead5_raw.fillna(lead4)
        df[f'lead4_{col}'] = lead4
        df[f'lead5_{col}'] = lead5

    # [NEW] lead_max3/lead_mean3 (t+1~3 요약) 핵심 4개 컬럼
    for col in ['congestion_score', 'low_battery_ratio', 'order_inflow_15m', 'charge_queue_length']:
        if f'lead1_{col}' in df.columns:
            trio = df[[f'lead1_{col}', f'lead2_{col}', f'lead3_{col}']]
            df[f'lead_max3_{col}']  = trio.max(axis=1)
            df[f'lead_mean3_{col}'] = trio.mean(axis=1)
            df[f'lead_min3_{col}']  = trio.min(axis=1)

    # Lead 기반 복합 위험 신호 (step35 동일)
    ...
    # [NEW] 추가 복합 위험 신호
    if 'lead1_congestion_score' in df.columns and 'lead1_order_inflow_15m' in df.columns:
        df['lead1_congestion_x_order'] = df['lead1_congestion_score'] * df['lead1_order_inflow_15m']
    if 'lead1_low_battery_ratio' in df.columns and 'lead1_charge_queue_length' in df.columns:
        df['lead1_battery_x_queue'] = df['lead1_low_battery_ratio'] * df['lead1_charge_queue_length']
    if 'lead1_max_zone_density' in df.columns and 'lead1_pack_utilization' in df.columns:
        df['lead1_density_x_pack'] = df['lead1_max_zone_density'] * df['lead1_pack_utilization']
...
print("  [NEW] lead 피처 (18개 컬럼 × lead1~5, NaN fill)...")
train = add_lead_features(train)
test  = add_lead_features(test)

# [NEW] Remaining 피처 (sc_* 필요)
print("  [NEW] remaining 피처 (10개 컬럼, 나머지 슬롯 통계)...")
...
run_count   = 0
total_runs  = len(SEEDS) + len(XGB_SEEDS) + len(CAT_SEEDS)  # LGB + XGB + CAT
...
for seed in SEEDS:
    ...
    for fold, (tr_idx, val_idx) in enumerate(gkf.split(X_train, y_train, groups)):
        ...
        fold_pred = np.clip(np.expm1(model.predict(X_val, num_iteration=model.best_iteration)), 0, None)
        oof_p[val_idx] = fold_pred
        fold_mae = mean_absolute_error(y_true[val_idx], fold_pred)
        test_p        += np.clip(np.expm1(model.predict(X_test, num_iteration=model.best_iteration)), 0, None) / N_SPLITS
        print(f"    fold{fold}: MAE={fold_mae:.4f} | iter={model.best_iteration}")
    sc = mean_absolute_error(y_true, oof_p)
    all_oof['LightGBM'][seed]   = oof_p
    all_test['LightGBM'][seed]  = test_p
    all_score['LightGBM'][seed] = sc
    run_count += 1
    print(f"  seed={seed} OOF: {sc:.4f} | {(time.time()-t0)/60:.1f}min")
...
for seed in XGB_SEEDS:
    ...
    for fold, (tr_idx, val_idx) in enumerate(gkf.split(X_train, y_train, groups)):
        ...
        model  = xgb.train(params, dtrain, num_boost_round=MAX_ROUNDS,
                           ...
                           verbose_eval=False)
        fold_pred = np.clip(np.expm1(model.predict(dvalid)), 0, None)
        oof_p[val_idx] = fold_pred
        fold_mae = mean_absolute_error(y_true[val_idx], fold_pred)
        test_p  += np.clip(np.expm1(model.predict(xgb.DMatrix(X_test))), 0, None) / N_SPLITS
        print(f"    fold{fold}: MAE={fold_mae:.4f} | iter={model.best_iteration}")
    sc = mean_absolute_error(y_true, oof_p)
...
for seed in CAT_SEEDS:
    ...
    for fold, (tr_idx, val_idx) in enumerate(gkf.split(X_train, y_train, groups)):
        X_tr, X_val = X_train.iloc[tr_idx], X_train.iloc[val_idx]
        y_tr, y_val = y_train[tr_idx], y_train[val_idx]
        model = CatBoostRegressor(**params, iterations=MAX_ROUNDS, early_stopping_rounds=EARLY_STOP)
        model.fit(X_tr, y_tr, eval_set=(X_val, y_val), use_best_model=True, verbose=False)
        fold_pred = np.clip(np.expm1(model.predict(X_val)), 0, None)
        oof_p[val_idx] = fold_pred
        fold_mae = mean_absolute_error(y_true[val_idx], fold_pred)
        test_p  += np.clip(np.expm1(model.predict(X_test)), 0, None) / N_SPLITS
        print(f"    fold{fold}: MAE={fold_mae:.4f} | iter={model.best_iteration_}")
    sc = mean_absolute_error(y_true, oof_p)
...
print(f"  CAT 평균: {np.mean(s):.4f} ± {np.std(s):.4f}")
...
oof_lgb  = np.stack(list(all_oof['LightGBM'].values())).mean(axis=0)
test_lgb = np.stack(list(all_test['LightGBM'].values())).mean(axis=0)
mae_lgb  = mean_absolute_error(y_true, oof_lgb)
print(f"  LightGBM 시드평균 OOF MAE: {mae_lgb:.4f}")

oof_xgb  = np.stack(list(all_oof['XGBoost'].values())).mean(axis=0)
test_xgb = np.stack(list(all_test['XGBoost'].values())).mean(axis=0)
mae_xgb  = mean_absolute_error(y_true, oof_xgb)
print(f"  XGBoost  시드평균 OOF MAE: {mae_xgb:.4f}")

# LGB:XGB = 5:3 비율 앙상블 (seed 수 비례)
oof_cat  = np.stack(list(all_oof['CatBoost'].values())).mean(axis=0)
test_cat = np.stack(list(all_test['CatBoost'].values())).mean(axis=0)
mae_cat  = mean_absolute_error(y_true, oof_cat)
print(f"  CatBoost 시드평균 OOF MAE: {mae_cat:.4f}")

n_lgb, n_xgb, n_cat = len(SEEDS), len(XGB_SEEDS), len(CAT_SEEDS)
total_n = n_lgb + n_xgb + n_cat
w_lgb = n_lgb / total_n
w_xgb = n_xgb / total_n
w_cat = n_cat / total_n
oof_blend  = w_lgb * oof_lgb  + w_xgb * oof_xgb  + w_cat * oof_cat
test_blend = w_lgb * test_lgb + w_xgb * test_xgb  + w_cat * test_cat
mae_blend  = mean_absolute_error(y_true, oof_blend)
print(f"  LGB+XGB+CAT 앙상블 OOF MAE: {mae_blend:.4f} (w={w_lgb:.2f}/{w_xgb:.2f}/{w_cat:.2f})")

best_mae   = mae_blend
best_preds = test_blend
oof_final  = oof_blend
s = list(all_score['LightGBM'].values())
print(f"\nStep45 OOF MAE: {best_mae:.4f}")
print(f"Step36 베이스:  8.5261 (3모델 앙상블)")
print(f"Step37 LGB단독: 8.5326 (비교 기준)")
print(f"차이 vs step37: {best_mae - 8.5326:+.4f}")
...
print("="*60)
# oof_final already set above (= oof_lgb)
...
print("8. Step 비교")
print("="*60)
print(f"  Step32 OOF MAE: 8.5729  (469 피처)")
print(f"  Step35 OOF MAE: 8.5323  (570 피처, +lead/remaining)")
print(f"  Step49 OOF MAE: {best_mae:.4f}  ({len(feature_cols)} 피처, Erlang-C+LGB+XGB+CAT)")
print(f"  Step35→36 변화: {best_mae - 8.5323:+.4f}")
print(f"\n  Step35 Public: 10.0533")
print(f"  Step49 예상 Public: ~{best_mae + 1.38:.4f}  (갭 1.38 목표)")
...
oof_path = os.path.join(OUTPUT_DIR, 'oof_step49.csv')
...
out = os.path.join(OUTPUT_DIR, 'submission_step49_ensemble.csv')
submission.to_csv(out, index=False)
print(f"제출 파일: submission_step49_ensemble.csv")
