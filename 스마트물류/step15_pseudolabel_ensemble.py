...
# Step 15: Pseudo-labeling (Big Bet)
# =============================================================================
# Step14 대비 변경점:
#
# [Pseudo-labeling]
# - OOF↔Public 갭 1.47의 근본 원인 = train/test 분포 차이
# - Step14 test 예측값을 pseudo-label로 활용
# - 각 fold 학습 시: 실제 train fold + 전체 test (가중치 0.3) 동시 학습
# - Validation: 실제 train val fold만 (pseudo 행 제외 → OOF 신뢰성 유지)
# - 결과: 모델이 test 분포를 직접 학습 → Public 갭 축소 기대
#
# 학습 구조:
#   학습 입력 = X_train[tr_idx] (weight=1.0) + X_test (weight=PSEUDO_WEIGHT)
#   검증 입력 = X_train[val_idx] (real labels only)
#   OOF      = original train 행만 계산 (신뢰 가능한 지표)
#
# 피처/모델/lr: Step14 완전 동일 (300 피처, LGB lr=0.02, XGB/Cat lr=0.05)
#
# 실행:
#   caffeinate -i nohup python step15_pseudolabel_ensemble.py > step15_output.log 2>&1 &
#   tail -f step15_output.log
# =============================================================================
...
LGB_MAX_ROUNDS = 5000     # LGB 전용 (lr 낮춰서 더 많은 라운드)
LGB_EARLY_STOP = 200      # LGB 전용
PSEUDO_WEIGHT  = 0.3      # pseudo-label 행 가중치 (실제 train=1.0 대비)
...
# ── [Pseudo-label 로드] ────────────────────────────────────────────────────────
print("  [Pseudo] Step14 예측 로드 중...")
pseudo_path = os.path.join(DATA_DIR, 'submission_step14_mixed_lr_ensemble.csv')
pseudo_sub  = pd.read_csv(pseudo_path)
y_pseudo_log = np.log1p(pseudo_sub[TARGET].values)
print(f"  Pseudo-label 로드 완료: {len(y_pseudo_log)}행 | 평균={np.expm1(y_pseudo_log).mean():.2f}분")
...
# =============================================================================
# 5. 모델 파라미터 (LGB만 lr=0.02, XGB/Cat는 Step12 동일)
# =============================================================================
LGB_BASE = {
    'objective': 'regression_l1', 'metric': 'mae',
    'learning_rate': 0.02, 'num_leaves': 127, 'max_depth': -1,  # lr: 0.05→0.02
...
# LightGBM + Pseudo-labeling
print("\nLightGBM × 멀티시드 (pseudo-labeling PSEUDO_WEIGHT={})".format(PSEUDO_WEIGHT))
for seed in SEEDS:
    t0 = time.time()
    ...
    for fold, (tr_idx, val_idx) in enumerate(gkf.split(X_train, y_train, groups)):
        ...
        # Pseudo: train fold + 전체 test (가중치 PSEUDO_WEIGHT)
        X_tr_ps = pd.concat([X_tr, X_test], ignore_index=True)
        y_tr_ps = np.concatenate([y_tr, y_pseudo_log])
        w_tr_ps = np.concatenate([np.ones(len(y_tr)), np.full(len(X_test), PSEUDO_WEIGHT)])
        dtrain  = lgb.Dataset(X_tr_ps, label=y_tr_ps, weight=w_tr_ps, free_raw_data=True)
...
for seed in SEEDS:
    ...
    for fold, (tr_idx, val_idx) in enumerate(gkf.split(X_train, y_train, groups)):
        ...
        y_tr_ps = np.concatenate([y_tr, y_pseudo_log])
        w_tr_ps = np.concatenate([np.ones(len(y_tr)), np.full(len(X_test), PSEUDO_WEIGHT)])
        dtrain  = xgb.DMatrix(X_tr_ps, label=y_tr_ps, weight=w_tr_ps)
...
for seed in SEEDS:
    ...
    for fold, (tr_idx, val_idx) in enumerate(gkf.split(X_train, y_train, groups)):
        ...
        model.fit(X_tr_ps, y_tr_ps, eval_set=(X_val, y_val),
                  sample_weight=w_tr_ps, use_best_model=True, verbose=False)
...
print(f"  Step13 OOF MAE: 8.6083  (365 피처, lr=0.02 균일)")
print(f"  Step14 OOF MAE: 8.6037  (300 피처, LGB lr=0.02 / XGB+Cat lr=0.05, 기준)")
print(f"  Step15 OOF MAE: {best_mae:.4f}  (Step14 + pseudo-labeling PSEUDO_WEIGHT={PSEUDO_WEIGHT})")
print(f"  Step14→15 변화: {best_mae - 8.6037:+.4f}")
print(f"\n  Step14 Public: 10.0733  (현재 최고)")
print(f"  Step15 Public: 미제출 (OOF 갭 기준 예상 ~{best_mae + 1.469:.4f})")
print(f"  ※ pseudo-labeling은 OOF 갭 자체를 줄이는 효과 → Public 실제 개선폭이 OOF보다 클 수 있음")
...
out = os.path.join(OUTPUT_DIR, 'submission_step15_pseudolabel_ensemble.csv')
submission.to_csv(out, index=False)
print(f"\n제출 파일: submission_step15_pseudolabel_ensemble.csv")
