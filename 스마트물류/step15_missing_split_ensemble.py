...
# Step 15: 결측치 구조적 분리 앙상블 (Missing Value Structural Split)
# =============================================================================
# Step14 대비 변경점:
#
# [결측치 구조적 분리 전략]
...
#   caffeinate -i nohup python step15_missing_split_ensemble.py > step15_output.log 2>&1 &
...
print(f"  Step15 OOF MAE: {overall_mae:.4f}  (결측 구조 분리, 2그룹 독립 앙상블)")
print(f"  Step14→15 변화: {overall_mae - 8.6037:+.4f}")
...
out = os.path.join(OUTPUT_DIR, 'submission_step15_missing_split_ensemble.csv')
submission.to_csv(out, index=False)
print(f"\n제출 파일: submission_step15_missing_split_ensemble.csv")
