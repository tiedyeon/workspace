...
#
# 실행:
#   caffeinate -i nohup python -u -B step36_lead_expanded.py > step36_output.log 2>&1 &
...
print(f"Step36: Lead 피처 확장 + NaN 처리 개선 버전")
...
print(f"  Step36 OOF MAE: {best_mae:.4f}  ({len(feature_cols)} 피처, +lead확장/NaN처리)")
print(f"  Step35→36 변화: {best_mae - 8.5323:+.4f}")
print(f"\n  Step35 Public: 10.0533")
print(f"  Step36 예상 Public: ~{best_mae + 1.52:.4f}  (갭 1.52 가정)")
...
oof_path = os.path.join(OUTPUT_DIR, 'oof_step36.csv')
...
out = os.path.join(OUTPUT_DIR, 'submission_step36_lead_expanded.csv')
submission.to_csv(out, index=False)
print(f"제출 파일: submission_step36_lead_expanded.csv")
