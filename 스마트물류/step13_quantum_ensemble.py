...
# Step 13: 퀀텀점프 시도 — 누락 sc_* + 트렌드 + suffix + 학습률 개선
# =============================================================================
# Step12 대비 변경점:
#
# [NEW 1] AGG_COLS 확장 (sc_* 집계 누락 피처 추가)
#     - battery_std: sc_mean 상관계수 0.359 (Step12까지 집계 안 됨!)
#     - sku_concentration: sc_mean 0.292
#     - urgent_order_ratio: sc_mean 0.292
#     - kpi_otd_pct: sc_mean -0.135
#
# [NEW 2] 시나리오 트렌드(slope) 피처
#     - sc_slope_{col} = 후반부(slot 13~24) 평균 - 전반부(slot 0~11) 평균
#     - sc_slope_charge_queue: corr=+0.243 (상승 = 위험 신호)
#     - sc_slope_robot_active: corr=-0.224 (감소 = 위험 신호)
#     - 기존 sc_mean/std/max/min 어디에도 없는 완전히 새로운 신호
#
# [NEW 3] suffix(미래 슬롯) 피처
#     - suf_mean_{col}: 현재 slot+1 ~ slot24 평균 (앞으로 남은 값들)
#     - suf_max_{col}: 현재 slot+1 ~ slot24 최대값
#     - 가능한 이유: 이 데이터는 시나리오 전체 25슬롯이 한 번에 주어짐
#       (sc_* 집계가 이미 전체 25슬롯을 쓰는 것과 동일한 논리)
#     - suf_mean_order_inflow: corr=0.368 vs 현재값 0.342 (+0.026)
#     - suf_mean_low_battery: corr=0.374 vs 현재값 0.366 (+0.008)
#
# [NEW 4] 학습률 0.05 → 0.02, MAX_ROUNDS 3000 → 5000
#     - Step12 seed=123 분석: 999라운드까지 학습된 시드가 가장 낮은 OOF 달성
#     - 모든 시드가 충분히 학습할 수 있도록 학습률 낮추고 라운드 늘림
#
# 실행:
#   caffeinate -i nohup python step13_quantum_ensemble.py > step13_output.log 2>&1 &
#   tail -f step13_output.log
# =============================================================================
...
N_SPLITS   = 5
MAX_ROUNDS = 5000    # 3000 → 5000 (충분한 학습 보장)
EARLY_STOP = 200     # 150 → 200 (낮은 lr에 맞게)
...
# [NEW] battery_std, sku_concentration, urgent_order_ratio, kpi_otd_pct 추가
AGG_COLS = [
    ...
    # Step13 신규 추가
    'battery_std', 'sku_concentration', 'urgent_order_ratio', 'kpi_otd_pct',
]
...
ZSCORE_COLS = ['congestion_score', 'low_battery_ratio', 'max_zone_density',
               ...
               'battery_std', 'sku_concentration', 'urgent_order_ratio']
...
# [NEW] suffix / slope 피처 대상
SUFFIX_COLS = ['congestion_score', 'low_battery_ratio', 'order_inflow_15m',
               'robot_active', 'blocked_path_15m', 'charge_queue_length']

SLOPE_COLS  = ['congestion_score', 'low_battery_ratio', 'order_inflow_15m',
               'robot_active', 'blocked_path_15m', 'charge_queue_length']
...
def feature_engineering(df: pd.DataFrame) -> pd.DataFrame:
    """Step11/12 피처 전체 (NaN 처리, 파생, log변환, 클러스터, 교호작용)"""
    df = df.copy()
    ...
    df['congestion_level'] = pd.cut(
        df['congestion_score'].fillna(0), bins=[-1, 0, 20, 100], labels=[0, 1, 2]
    ).astype(int)
...
def add_suffix_features(df: pd.DataFrame) -> pd.DataFrame:
    """[NEW] suffix 피처: 현재 slot 이후 남은 슬롯들의 누적 통계
    sc_sum - exp_sum = 아직 오지 않은 슬롯들의 합
    suf_mean = suf_sum / remaining_slots
    """
    df = df.copy()
    for col in SUFFIX_COLS:
        if col not in df.columns:
            continue
        exp_sum = df.groupby('scenario_id')[col].expanding().sum().reset_index(level=0, drop=True)
        sc_sum  = df.groupby('scenario_id')[col].transform('sum')
        sc_max  = df.groupby('scenario_id')[col].transform('max')

        suf_sum       = sc_sum - exp_sum
        remaining     = 24 - df['slot_idx']

        suf_mean = suf_sum / (remaining + 1e-5)
        suf_mean[remaining == 0] = df.loc[remaining == 0, col]  # slot24: 현재값으로

        # suffix max는 sc_max와 같지 않음 — 역방향 expanding으로 계산
        # reversed expanding max within group
        rev_max = (df.groupby('scenario_id')[col]
                     .transform(lambda x: x[::-1].expanding().max()[::-1]))
        suf_max = rev_max.shift(-1, fill_value=np.nan)
        suf_max = df.groupby('scenario_id')[col].transform(
            lambda x: x[::-1].shift(1).expanding().max()[::-1]
        )

        df[f'suf_mean_{col}'] = suf_mean
        df[f'suf_max_{col}']  = suf_max

        # suffix vs expanding 비율 (앞으로가 지금까지보다 더 나쁜가?)
        exp_mean = df.get(f'exp_mean_{col}')
        if exp_mean is not None:
            df[f'suf_vs_exp_{col}'] = suf_mean / (exp_mean + 1e-5)

    return df


def add_slope_features(train_df: pd.DataFrame, test_df: pd.DataFrame):
    """[NEW] 시나리오 트렌드 피처: 상반부 vs 하반부 평균 차이
    양수 = 악화 중 (후반부가 더 심각)
    음수 = 개선 중 (후반부가 덜 심각)
    sc_slope_charge_queue_length: corr=+0.243
    sc_slope_robot_active:        corr=-0.224
    """
    slope_maps = {}
    for col in SLOPE_COLS:
        if col not in train_df.columns:
            continue
        early = (train_df[train_df['slot_idx'] <= 11]
                 .groupby('scenario_id')[col].mean())
        late  = (train_df[train_df['slot_idx'] >= 13]
                 .groupby('scenario_id')[col].mean())
        slope = (late - early).rename(f'sc_slope_{col}')
        slope_maps[f'sc_slope_{col}'] = slope

    slope_df = pd.DataFrame(slope_maps)
    train_df = train_df.merge(slope_df.reset_index(), on='scenario_id', how='left')

    # test: slope는 test 자체 데이터로 계산 (시나리오 전체 데이터 있으니까)
    slope_maps_test = {}
    for col in SLOPE_COLS:
        if col not in test_df.columns:
            continue
        early = (test_df[test_df['slot_idx'] <= 11]
                 .groupby('scenario_id')[col].mean())
        late  = (test_df[test_df['slot_idx'] >= 13]
                 .groupby('scenario_id')[col].mean())
        slope_maps_test[f'sc_slope_{col}'] = (late - early)

    slope_df_test = pd.DataFrame(slope_maps_test)
    test_df = test_df.merge(slope_df_test.reset_index(), on='scenario_id', how='left')

    return train_df, test_df


def add_aggregation_features(train_df, test_df):
    """sc_* 집계 피처 (AGG_COLS 확장 버전)"""
    valid_agg = [c for c in AGG_COLS if c in train_df.columns]

    train_sc = train_df.groupby('scenario_id')[valid_agg].agg(AGG_FUNCS)
    train_sc.columns = [f'sc_{c}_{f}' for c, f in train_sc.columns]
    test_sc  = test_df.groupby('scenario_id')[valid_agg].agg(AGG_FUNCS)
    ...
    layout_agg = train_df.groupby('layout_id')[valid_agg].agg(AGG_FUNCS)
...
def add_layout_target_encoding(train_df, test_df, y_log):
    ...
    print(f"  layout_te: train OOF 완료 | test cold-start {n_cold}행({n_cold/len(test_df)*100:.1f}%) fallback")
...
print("  [Step12] 시간 순서 피처 (slot_idx, expanding, lag)...")
train = add_temporal_features(train)
test  = add_temporal_features(test)

print("  [NEW] suffix(미래 슬롯) 피처...")
train = add_suffix_features(train)
test  = add_suffix_features(test)
...
print("  [Step12] Trajectory 피처...")
train = add_trajectory_features(train)
test  = add_trajectory_features(test)

print("  [NEW] 시나리오 트렌드(slope) 피처...")
train, test = add_slope_features(train, test)

y_log_full = np.log1p(train[TARGET].values)

print("  [Step12] Layout 타겟 인코딩 (OOF)...")
...
print(f"\nStep12 피처 수: 300개  →  Step13 피처 수: {len(feature_cols)}개 (+{len(feature_cols)-300}개)")
print(f"train: {X_train.shape} | test: {X_test.shape}")

# =============================================================================
# 5. 모델 파라미터 ([NEW] learning_rate 0.05 → 0.02)
# =============================================================================
LGB_BASE = {
...
# =============================================================================
print("\n" + "="*60)
print("3. 학습 시작 (lr=0.02, MAX_ROUNDS=5000)")
print("="*60)
...
print("5. Step 비교")
print("="*60)
print(f"  Step7  OOF MAE: 8.6852  (197 피처, lr=0.05)")
print(f"  Step11 OOF MAE: 8.6470  (267 피처, EDA 강화)")
print(f"  Step12 OOF MAE: 8.6050  (300 피처, 시간순서+Layout TE)")
print(f"  Step13 OOF MAE: {best_mae:.4f}  ({len(feature_cols)} 피처, suffix+slope+확장sc+lr0.02)")
print(f"  Step12→13 변화: {best_mae - 8.6050:+.4f}")
print(f"  Step7→13  변화: {best_mae - 8.6852:+.4f}")
print(f"\n  Step7  Public: 10.1823  (현재 최고)")
print(f"  Step12 Public: 미제출 (예상 ~10.1020)")
print(f"  Step13 Public: 미제출 (예상 ~{best_mae + 1.497:.4f})")
...
out = os.path.join(OUTPUT_DIR, 'submission_step13_quantum_ensemble.csv')
submission.to_csv(out, index=False)
print(f"\n제출 파일: submission_step13_quantum_ensemble.csv")
